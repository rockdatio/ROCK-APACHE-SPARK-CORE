...
if __name__ == '__main__':
    ...
    df = spark.read.format("csv").option("header", "true").load(
        "hdfs://0.0.0.0:19000/enrique/data/Ejemplos/ExportCSV.csv")
    ...
    df = df.selectExpr("*",
                       "case when (MONTH(Date)<=3) then concat(YEAR(Date) - 1,'Q3') when (MONTH(Date)<=6) then concat(YEAR(Date) - 1,'Q4') when (MONTH(Date)<=9) then concat(YEAR(Date) - 0,'Q1') ELSE concat(YEAR(Date) - 0,'Q2') end AS Quarter")
